...
class PointServiceImplTest {
	...
	void testGetPointHistories() {
		...
		when(pointHistoryRepository.selectAllByUserId(1L)).thenReturn(histories);
	...
	void testChargePoint() {
		// given
		when(userPointRepository.selectById(1L)).thenReturn(userPoint);
		when(userPointRepository.insertOrUpdate(1L, 150L)).thenReturn(new UserPoint(1L, 150L, System.currentTimeMillis()));
		when(pointHistoryRepository.insert(anyLong(), anyLong(), any(TransactionType.class), anyLong()))
	...
	void testUsePoint() {
		// given
		when(userPointRepository.selectById(1L)).thenReturn(userPoint);
		when(userPointRepository.insertOrUpdate(1L, 80L)).thenReturn(new UserPoint(1L, 80L, System.currentTimeMillis()));
		when(pointHistoryRepository.insert(anyLong(), anyLong(), any(TransactionType.class), anyLong()))
